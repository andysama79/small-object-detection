...
#     def __init__(self, in_channels, stride=2, upscaling_factor=2):
#         super(UpMerging, self).__init__()
#         self.pixel_shuffle = nn.PixelShuffle(2)  # PixelShuffle with stride 2
#         self.linear = nn.Linear(in_channels//(stride)**2, (in_channels//(stride)**2)*upscaling_factor)
...
class UpMerging(nn.Module):
    def __init__(self, in_channels, img_size, stride=2, upscaling_factor=2):
        super(UpMerging, self).__init__()
        ...
        self.linear = nn.Linear(input_size, (input_size)*upscaling_factor)
        self.stride = stride
    ...
    def forward(self, x):
        ...
        x = self.flatten(x)
        x = self.linear(x)
        x = rearrange(tensor=x, pattern= 'b (h w c) -> b c h w', h=self.img_size*self.stride, w=self.img_size*self.stride, c=self.in_channels//2)
        return x
    ...
    def __init__(self, in_channel, hid_dim, layers, down_scaling_factor, num_heads, head_dim, window_size, rel_pos_emb):
        ...
        for _ in range(layers//2):
            self.layers.append(nn.ModuleList([
                Swin_Block(dim=hid_dim*8, num_heads=num_heads, head_dim=head_dim, mlp_dim = hid_dim*4, shifted=False ,window_size=window_size, rel_pos_emb=rel_pos_emb),
        ...
        self.up_merge = UpMerging(in_channels=hid_dim, stride=2, upscaling_factor=2)

    def forward(self, x):
        # x = self.patch_partition(x)
        print("input shape", x.shape)
        for regular, shifted in self.layers:
            print("regular: ", regular, "shifted: ", shifted)
            x = regular(x)
        #     print("regular", x.shape)
        #     x = shifted(x)
        #     print("shifted", x.shape)
        return x.permute(0,3,1,2)

class SwinTransformer(nn.Module):
    def __init__(self, *, hid_dim, layers, heads, channels=3, num_classes=1000, head_dim=32, window_size=2, down_scaling_fact=(4,2,2,2), up_scaling_fact=(2,2,2,2), rel_pos_emb = True):
      super().__init__()
      self.stage1 = StageModule(in_channel = hid_dim*8, hid_dim=hid_dim, layers=layers[0], down_scaling_factor=down_scaling_fact[0], num_heads=heads[0], head_dim=head_dim, window_size=window_size, rel_pos_emb=rel_pos_emb)
...
def swin_t(hid_dim=96, layers=(2, 2, 2, 2), heads=(24, 12, 6, 24), **kwargs):
